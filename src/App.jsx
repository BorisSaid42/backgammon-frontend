...
// ═══════════════════════════════════════════════════════════════
// GAME LOGIC (client-side preview only — server re-validates)
// ═══════════════════════════════════════════════════════════════
const CHECKERS_PER_PLAYER = 15, BAR = "bar", OFF = "off", WHITE = 1, BLACK = -1;

// Board indices: 0=point1, 23=point24
// White (positive) has home at points 1-6 (indices 0-5), moves from high index to low
// Black (negative) has home at points 19-24 (indices 18-23), moves from low index to high
const INITIAL_BOARD = () => {
...
function Fireworks() {
  const p = useMemo(() => {
    const a = [];
    for (let i = 0; i < 48; i++) {
      const ang = (Math.PI * 2 * i) / 48 + (Math.random() - 0.5) * 0.5;
      const d = 60 + Math.random() * 160;
      a.push({ fx: Math.cos(ang) * d, fy: Math.sin(ang) * d, color: ['#f0c040','#e85050','#40c0f0','#60e060','#f080d0','#9945FF'][Math.floor(Math.random() * 6)], size: 4 + Math.random() * 7, delay: Math.random() * 0.4, dur: 0.8 + Math.random() * 0.6 });
    }
    return a;
  }, []);
  return (
    <div style={{ position: "absolute", top: "50%", left: "50%", pointerEvents: "none", zIndex: 10 }}>
      {p.map((p, i) => <div key={i} style={{ position: "absolute", width: p.size, height: p.size, borderRadius: "50%", background: p.color, boxShadow: `0 0 8px ${p.color}`, '--fx': `${p.fx}px`, '--fy': `${p.fy}px`, animation: `firework ${p.dur}s ${p.delay}s cubic-bezier(0.16,1,0.3,1) both` }} />)}
    </div>
  );
}
...
function DiceDisplay({ dice, usedDice = [], diceKey }) {
  ...
  for (const d of usedDice) {
    const idx = remaining.findIndex((v, i) => v === d && !usedIndices.has(i));
  ...
  return (
    <div style={{ display: "flex", gap: 10, justifyContent: "center", marginBottom: 10 }}>
      {dice.map((d, i) => {
        ...
        return (
          <div key={`${diceKey}-${i}`} className={used ? "" : "dice-bounce"} style={{
            width: 48, height: 48,
            background: used ? "#14202e" : "linear-gradient(145deg, #faf3e0, #e8d4a0)",
            border: used ? "1px solid #2a3f52" : "2px solid #c4a060",
            borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center",
            fontSize: 24, fontWeight: 700, fontFamily: "'Playfair Display', Georgia, serif",
            color: used ? "#3a4a5a" : "#1a1a1a", opacity: used ? 0.35 : 1,
            animationDelay: `${i * 0.12}s`,
            boxShadow: used ? "none" : "0 4px 16px rgba(0,0,0,0.35), inset 0 1px 0 rgba(255,255,255,0.3)",
...
function TxOverlay({ message }) {
  return (
    <div className="fade-in" style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", backdropFilter: "blur(6px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200 }}>
      <div className="slide-down" style={{ background: "linear-gradient(160deg,#1e2d3d,#162030)", border: "1px solid #3a5060", borderRadius: 16, padding: "40px 48px", textAlign: "center", maxWidth: 420, boxShadow: "0 24px 80px rgba(0,0,0,0.6)" }}>
        <div className="tx-spin" style={{ width: 48, height: 48, border: "3px solid #2a3f52", borderTopColor: "#9945FF", borderRadius: "50%", margin: "0 auto 20px" }} />
        <p style={{ color: "#e8d4a0", fontSize: 16, fontWeight: 600, margin: "0 0 8px", fontFamily: "'Playfair Display', serif" }}>{message}</p>
        <p style={{ color: "#556677", fontSize: 13, margin: 0 }}>Please confirm in your Phantom wallet...</p>
      </div>
    </div>
  );
}

function WalletButton({ wallet, onConnect, balance }) {
  if (wallet) {
    const short = wallet.slice(0, 4) + "..." + wallet.slice(-4);
    return (
      <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 14px", background: "#0d1520", borderRadius: 10, border: "1px solid #2a3f52" }}>
        <div style={{ width: 8, height: 8, borderRadius: "50%", background: "#14F195" }} />
        <span style={{ color: "#9945FF", fontSize: 13, fontWeight: 600, fontFamily: "monospace" }}>{short}</span>
        {balance !== null && <span style={{ color: "#556677", fontSize: 12 }}>{balance.toFixed(4)} SOL</span>}
      </div>
    );
  }
  return (
    <button className="btn-hover" onClick={onConnect} style={{ ...S.btn, background: "linear-gradient(135deg,#9945FF,#7B3FE4)", color: "#fff", fontSize: 13, padding: "10px 20px", display: "flex", alignItems: "center", gap: 8 }}>
...
// ═══════════════════════════════════════════════════════════════
// BOARD SVG — CORRECTED LAYOUT
// 
// Standard backgammon board layout (White's perspective):
//   Top row (left to right):    point 13, 14, 15, 16, 17, 18 | BAR | 19, 20, 21, 22, 23, 24
//   Bottom row (left to right): point 12, 11, 10, 9, 8, 7    | BAR | 6, 5, 4, 3, 2, 1
//
// White's home board = bottom-right (points 1-6)
// Black's home board = top-right (points 19-24)
// White moves: high→low (counterclockwise visually)
// Black moves: low→high (clockwise visually)
//
// Internal array: index 0 = point 1, index 23 = point 24
// ═══════════════════════════════════════════════════════════════
function BoardSVG({ board, barW, barB, selectedPoint, validDestinations, onPointClick, onBarClick, playerColor, flipped, cubeValue, cubeOwner, offW, offB }) {
  const W = 860, H = 600, M = 36, BW = 42;
  const PW = (W - M * 2 - BW) / 12, PH = 240, CR = Math.min(PW * 0.44, 24);
  const wC = "#f0e6d2", bC = "#2a2a2a";

  // Map board indices to visual positions
  // White's perspective (not flipped):
  //   Top row L→R:    indices 12,13,14,15,16,17 | bar | 18,19,20,21,22,23
  //   Bottom row L→R: indices 11,10,9,8,7,6     | bar | 5,4,3,2,1,0
  // Black's perspective (flipped): reverse everything
  ...
  function getX(vi) {
    if (vi < 6) return M + vi * PW + PW / 2;
    return M + 6 * PW + BW + (vi - 6) * PW + PW / 2;
  }

  function drawPoint(idx, vi, top) {
    const x = getX(vi), y = top ? M : H - M, dir = top ? 1 : -1;
    const isSel = selectedPoint === idx, isVal = validDestinations.includes(idx);
    const fill = idx % 2 === 0 ? "#b8895a" : "#6e5438";
    const pts = `${x - PW / 2 + 3},${y} ${x},${y + dir * PH} ${x + PW / 2 - 3},${y}`;
    const clickY = top ? M : H - M - PH;
    const pointNum = idx + 1; // point number = index + 1

    return (
      <g key={`p-${idx}`} onClick={() => onPointClick(idx)} style={{ cursor: "pointer" }}>
        <rect x={x - PW / 2} y={clickY} width={PW} height={PH} fill="transparent" />
        <polygon points={pts} fill={isVal ? "rgba(100,220,120,0.3)" : fill} opacity={0.85}>
          {isSel && <animate attributeName="opacity" values="0.7;1;0.7" dur="1.2s" repeatCount="indefinite" />}
        </polygon>
        {isSel && (
          <polygon points={pts} fill="none" stroke="#f0c040" strokeWidth={2.5}>
            <animate attributeName="stroke-opacity" values="0.4;1;0.4" dur="1.5s" repeatCount="indefinite" />
          </polygon>
        )}
        {isVal && (
          <circle cx={x} cy={y + dir * (CR + Math.abs(board[idx]) * CR * 1.85)} r={CR * 0.5} fill="rgba(100,220,120,0.4)" stroke="#6c6" strokeWidth={1.5}>
            <animate attributeName="r" values={`${CR * 0.4};${CR * 0.7};${CR * 0.4}`} dur="1.5s" repeatCount="indefinite" />
            <animate attributeName="opacity" values="0.3;0.9;0.3" dur="1.5s" repeatCount="indefinite" />
          </circle>
        )}
        {drawCheckers(board[idx], x, y, dir, idx)}
        <text x={x} y={top ? M - 8 : H - M + 16} textAnchor="middle" fill="#4a5a6a" fontSize="9" fontFamily="'DM Sans',sans-serif" fontWeight="500">
          {pointNum}
        </text>
      </g>
    );
  ...
  function drawCheckers(count, cx, baseY, dir, pointIdx) {
    if (count === 0) return null;
    const isW = count > 0, col = isW ? wC : bC, st = isW ? "#c4a882" : "#555";
    const n = Math.abs(count), items = [], ms = 5, isSel = selectedPoint === pointIdx;
    for (let i = 0; i < Math.min(n, ms); i++) {
      const cy = baseY + dir * (CR + i * CR * 1.85), isTop = i === Math.min(n, ms) - 1;
      items.push(
        <g key={i} style={isSel && isTop ? { filter: "drop-shadow(0 0 10px rgba(240,192,64,0.6))" } : {}}>
          <ellipse cx={cx + 1} cy={cy + 2} rx={CR} ry={CR * 0.25} fill="rgba(0,0,0,0.12)" />
          <circle cx={cx} cy={cy} r={CR} fill={col} stroke={st} strokeWidth={1.5}>
            {isSel && isTop && <animate attributeName="r" values={`${CR};${CR + 1.5};${CR}`} dur="0.8s" repeatCount="indefinite" />}
          </circle>
          <circle cx={cx} cy={cy} r={CR - 2} fill="none" stroke={isW ? "rgba(255,255,255,0.2)" : "rgba(255,255,255,0.06)"} strokeWidth={1} />
          {isW && <circle cx={cx} cy={cy} r={CR * 0.55} fill="none" stroke={st} strokeWidth={0.8} opacity={0.35} />}
          <circle cx={cx - CR * 0.25} cy={cy - CR * 0.25} r={CR * 0.13} fill={isW ? "rgba(255,255,255,0.35)" : "rgba(255,255,255,0.08)"} />
          {isSel && isTop && (
            <circle cx={cx} cy={cy} r={CR + 4} fill="none" stroke="#f0c040" strokeWidth={2}>
              <animate attributeName="r" values={`${CR + 3};${CR + 7};${CR + 3}`} dur="1.2s" repeatCount="indefinite" />
    ...
    if (n > ms) {
      const cy = baseY + dir * (CR + (ms - 1) * CR * 1.85);
      items.push(<text key="c" x={cx} y={cy + 5} textAnchor="middle" fill={isW ? "#333" : "#ddd"} fontSize="13" fontWeight="bold">{n}</text>);
    }
    return items;
  }
  ...
  function drawBar() {
    ...
    for (let i = 0; i < barW; i++)
      items.push(<circle key={`bw-${i}`} cx={bx + BW / 2} cy={H / 2 + 40 + i * 28} r={CR * 0.9} fill={wC} stroke="#c4a882" strokeWidth={1.5} onClick={() => onBarClick(WHITE)} style={{ cursor: "pointer" }} />);
    for (let i = 0; i < barB; i++)
      items.push(<circle key={`bb-${i}`} cx={bx + BW / 2} cy={H / 2 - 40 - i * 28} r={CR * 0.9} fill={bC} stroke="#555" strokeWidth={1.5} onClick={() => onBarClick(BLACK)} style={{ cursor: "pointer" }} />);
    return items;
  }

  function drawOff() {
    const items = [], ox = W - 22;
    // White bears off bottom-right (when not flipped), Black bears off top-right
    const wOffY = flipped ? M : H - M;
    const bOffY = flipped ? H - M : M;
    for (let i = 0; i < offW; i++)
      items.push(<rect key={`ow-${i}`} x={ox - 12} y={wOffY - (flipped ? -i * 9 : 8 + i * 9)} width={24} height={7} rx={2} fill={wC} stroke="#c4a882" strokeWidth={0.5} opacity={0} style={{ animation: `fadeIn 0.3s ${i * 0.04}s both` }} />);
    for (let i = 0; i < offB; i++)
      items.push(<rect key={`ob-${i}`} x={ox - 12} y={bOffY + (flipped ? -(8 + i * 9) : i * 9)} width={24} height={7} rx={2} fill={bC} stroke="#555" strokeWidth={0.5} opacity={0} style={{ animation: `fadeIn 0.3s ${i * 0.04}s both` }} />);

    if (validDestinations.includes(OFF)) {
      const oy = playerColor === WHITE ? (flipped ? M : H - M - 140) : (flipped ? H - M - 140 : M);
      items.push(
        <g key="off" onClick={() => onPointClick(OFF)} style={{ cursor: "pointer" }}>
          <rect x={ox - 18} y={oy} width={36} height={140} rx={6} fill="rgba(100,220,120,0.2)" stroke="#6c6" strokeWidth={2} strokeDasharray="6 4">
            <animate attributeName="stroke-opacity" values="0.4;1;0.4" dur="1.5s" repeatCount="indefinite" />
          </rect>
          <text x={ox} y={oy + 75} textAnchor="middle" fill="#6c6" fontSize="11" fontWeight="600">OFF</text>
        </g>
      );
    }
    return items;
  ...
  function drawCube() {
    let cx, cy;
    if (cubeOwner === 0) { cx = M + 6 * PW + BW / 2; cy = H / 2; }
    else if (cubeOwner === WHITE) { cx = 16; cy = flipped ? M + 60 : H - M - 60; }
    else { cx = 16; cy = flipped ? H - M - 60 : M + 60; }
    return (
      <g className="glow-pulse">
        <rect x={cx - 16} y={cy - 16} width={32} height={32} rx={5} fill="#2c1810" stroke="#aa8844" strokeWidth={1.5} />
        <text x={cx} y={cy + 6} textAnchor="middle" fill="#e8d4a0" fontSize="15" fontWeight="bold" fontFamily="'Playfair Display',Georgia,serif">{cubeValue}</text>
      </g>
    );
  }

  // Filter nulls and map to visual indices
  const topPoints = topRow.filter(x => x !== null);
  const botPoints = botRow.filter(x => x !== null);

  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", maxWidth: 920, display: "block", margin: "0 auto", filter: "drop-shadow(0 8px 32px rgba(0,0,0,0.4))" }}>
      <defs>
        <linearGradient id="bg" x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stopColor="#1e2d3d" /><stop offset="100%" stopColor="#162030" /></linearGradient>
        <linearGradient id="fg" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#2a3f52" /><stop offset="100%" stopColor="#1e3040" /></linearGradient>
        <linearGradient id="br" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#14202e" /><stop offset="100%" stopColor="#0f1820" /></linearGradient>
      </defs>
      <rect x={0} y={0} width={W} height={H} rx={14} fill="url(#bg)" />
      <rect x={M - 6} y={M - 6} width={W - M * 2 + 12} height={H - M * 2 + 12} rx={6} fill="none" stroke="#3a5060" strokeWidth={1} opacity={0.4} />
      <rect x={M - 2} y={M - 2} width={W - M * 2 + 4} height={H - M * 2 + 4} rx={4} fill="url(#fg)" stroke="#2a4050" strokeWidth={1} />
      <rect x={M + 6 * PW} y={M - 2} width={BW} height={H - M * 2 + 4} fill="url(#br)" />
      <line x1={M} y1={H / 2} x2={M + 6 * PW} y2={H / 2} stroke="#2a3a4a" strokeWidth={0.5} opacity={0.3} />
      <line x1={M + 6 * PW + BW} y1={H / 2} x2={W - M} y2={H / 2} stroke="#2a3a4a" strokeWidth={0.5} opacity={0.3} />
      {topPoints.map((idx, vi) => drawPoint(idx, vi, true))}
      {botPoints.map((idx, vi) => drawPoint(idx, vi, false))}
      {drawBar()}
      {drawOff()}
      {drawCube()}
    </svg>
...
function DoublingDialog({ type, cubeValue, onAccept, onReject, playerName, wagerPerPoint }) {
  ...
  return (
    <div className="fade-in" style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100 }}>
      <div className="slide-down" style={{ background: "linear-gradient(160deg,#1e2d3d,#162030)", border: "1px solid #3a5060", borderRadius: 16, padding: "36px 44px", textAlign: "center", maxWidth: 420, boxShadow: "0 24px 80px rgba(0,0,0,0.6)" }}>
        <div className="spin-in" style={{ fontSize: 48, marginBottom: 16 }}>🎲</div>
        {type === "double" ? (<>
          <h3 className="shimmer-text" style={{ margin: "0 0 8px", fontFamily: "'Playfair Display',Georgia,serif", fontSize: 22 }}>{playerName} doubles!</h3>
          <p style={{ color: "#8899aa", fontSize: 14, margin: "0 0 8px" }}>Stakes raised to <strong style={{ color: "#f0c040" }}>{cubeValue * 2}</strong> points</p>
          {wagerPerPoint > 0 && <p className="sol-pulse" style={{ color: "#9945FF", fontSize: 13, margin: "0 0 20px" }}>Accepting costs <strong>{cost.toFixed(4)} SOL</strong></p>}
          <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
            <button onClick={onReject} className="btn-hover" style={bSt("#8b3030")}>Drop ({cubeValue}pt)</button>
            <button onClick={() => onAccept(false)} className="btn-hover" style={bSt("#2e6b3e")}>Take{wagerPerPoint > 0 ? ` (${cost.toFixed(4)} SOL)` : ""}</button>
            <button onClick={() => onAccept(true)} className="btn-hover" style={bSt("#6b5a2e")}>Beaver!{wagerPerPoint > 0 ? ` (${(cost * 2).toFixed(4)} SOL)` : ""}</button>
          </div>
        </>) : (<>
          <h3 className="shimmer-text" style={{ margin: "0 0 8px", fontFamily: "'Playfair Display',Georgia,serif", fontSize: 22 }}>{playerName} beavers!</h3>
          <p style={{ color: "#8899aa", fontSize: 14, margin: "0 0 8px" }}>Stakes now <strong style={{ color: "#f0c040" }}>{newCube}</strong> points.</p>
          {wagerPerPoint > 0 && <p className="sol-pulse" style={{ color: "#9945FF", fontSize: 13, margin: "0 0 20px" }}>Accepting costs <strong>{cost.toFixed(4)} SOL</strong></p>}
          <div style={{ display: "flex", gap: 12, justifyContent: "center" }}>
            <button onClick={onReject} className="btn-hover" style={bSt("#8b3030")}>Drop ({newCube / 2}pt)</button>
            <button onClick={() => onAccept(false)} className="btn-hover" style={bSt("#2e6b3e")}>Accept{wagerPerPoint > 0 ? ` (${cost.toFixed(4)} SOL)` : ""}</button>
          </div>
        </>)}
      </div>
    </div>
...
export default function App() {
  ...
  async function payAndCall(amountSol, apiPath, bodyWithoutSig) {
    ...
    try {
      const result = await api(apiPath, { method: "POST", body: { ...bodyWithoutSig, txSignature } });
      setTxStatus(null);
  ...
  async function createLobby() {
    ...
    try {
      const result = await payAndCall(wager, "/lobby/create", { playerName: nameInput.trim(), wallet: walletAddr || "", wagerPerPoint: wager });
      setPlayerName(nameInput.trim()); setLobbyId(result.lobbyId); setPlayerId(result.playerId); setMyColor(result.color); setScreen("lobby"); setError("");
    } catch (e) { setError(e.message); }
  }
  ...
  async function joinLobby() {
    ...
    try {
      ...
      const result = await payAndCall(wager, `/lobby/${code}/join`, { playerName: nameInput.trim(), wallet: walletAddr || "" });
      setPlayerName(nameInput.trim()); setLobbyId(code); setPlayerId(result.playerId); setMyColor(result.color); setScreen("lobby"); setError("");
    } catch (e) { setError(e.message); }
  }
  ...
  async function startGame() {
    try {
      ...
      setLobby(result); setDiceKey(k => k + 1); setShowFireworks(false); setPendingMoves([]);
    } catch (e) { setError(e.message); }
  }
  ...
  const pollGame = useCallback(async () => {
    ...
    try {
      ...
      if (data.game && prevTurnRef.current !== null && data.game.turn !== prevTurnRef.current) { setDiceKey(k => k + 1); setPendingMoves([]); }
  ...
  function handlePointClick(pi) {
    if (!isMyTurn || game.phase !== "move") return;
    // Priority: if selected and clicking a valid destination, move there
      ...
      <div className="bg-animate" style={S.container}>
        ...
        <div className="fade-in-up card-hover" style={S.homeCard}>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <input className="input-focus" style={S.input} value={nameInput} onChange={e => setNameInput(e.target.value)} placeholder="Enter your name" maxLength={20} onKeyDown={e => e.key === "Enter" && createLobby()} />
          </div>
          <div style={{ marginBottom: 20 }}>
            <label style={S.label}>Wager per Point (SOL)</label>
            <div style={{ display: "flex", gap: 8 }}>
              {["0", "0.01", "0.05", "0.1", "0.5"].map(v => (
                <button key={v} className="btn-hover" onClick={() => setWagerAmount(v)} style={{ ...S.btn, ...(wagerAmount === v ? { background: "#9945FF", color: "#fff" } : { background: "#1e2d3d", color: "#7a8a9a", border: "1px solid #2a3f52" }), padding: "8px 14px", fontSize: 13, flex: 1 }}>
                  {v === "0" ? "Free" : v + " ◎"}
                </button>
              ))}
            </div>
            <input className="input-focus" style={{ ...S.input, marginTop: 8 }} value={wagerAmount} onChange={e => setWagerAmount(e.target.value)} placeholder="Custom" type="number" step="0.001" min="0" />
          </div>
          <button className="btn-hover" style={{ ...S.btn, ...S.btnP, width: "100%", marginBottom: 16 }} onClick={createLobby}>
            Create Lobby{parseFloat(wagerAmount) > 0 ? ` (Pay ${wagerAmount} SOL)` : ""}
          </button>
          <div style={S.divider}><span style={S.dividerText}>or join a game</span></div>
          <div style={{ marginBottom: 16 }}>
            <label style={S.label}>Lobby Code</label>
            <input className="input-focus" style={S.input} value={joinCode} onChange={e => setJoinCode(e.target.value)} placeholder="Paste lobby code" onKeyDown={e => e.key === "Enter" && joinLobby()} />
          </div>
          ...
          <div style={S.rules}><strong style={{ color: "#9945FF" }}>How it works:</strong> Players pay wager to a secure house wallet. Server verifies every payment on-chain. Winner receives the full pot automatically. 0% commission.</div>
        </div>
      </div>
    );
  }
      ...
      <div className="bg-animate" style={S.container}>
        ...
        <div className="fade-in-up" style={S.homeCard}>
          ...
          <div className="fade-in" style={S.lobbyInfo}>
            <div style={{ marginBottom: 8 }}><span style={{ color: "#8899aa", fontSize: 13 }}>Send this link to your opponent:</span></div>
            <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
              <code style={{ ...S.code, fontSize: 12, wordBreak: "break-all", flex: 1 }}>{typeof window !== 'undefined' ? `${window.location.origin}?join=${lobbyId}` : lobbyId}</code>
              <button onClick={copyLink} className="btn-hover" style={{ ...S.btn, ...S.btnSm, fontSize: 12, whiteSpace: "nowrap" }}>{copied ? "Copied!" : "Copy Link"}</button>
            </div>
            {wagerPerPoint > 0 && <div className="sol-pulse" style={{ color: "#9945FF", fontSize: 14, fontWeight: 600, marginTop: 8 }}>◎ {wagerPerPoint} SOL per point · Pot: {lobby?.totalPot?.toFixed(4) || 0} SOL</div>}
            {wagerPerPoint === 0 && <div style={{ color: "#556677", fontSize: 13, marginTop: 8 }}>Free game</div>}
          </div>
  ...
  return (
    <div className="bg-animate fade-in" style={{ ...S.container, justifyContent: "flex-start", paddingTop: 16 }}>
      ...
      <div className="fade-in-up" style={S.gameHeader}>
        <div className={isMyTurn ? "turn-glow" : ""} style={S.scoreBoard}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <div>
              <div style={{ color: "#f0e6d2", fontWeight: 600, fontSize: 14 }}>{hostName}</div>
              {game.turn === WHITE && <div style={{ color: "#f0c040", fontSize: 10, textTransform: "uppercase" }}>Playing</div>}
            </div>
          </div>
          <div style={{ textAlign: "center" }}>
            <div className="shimmer-text" style={{ fontWeight: 700, fontSize: 24, fontFamily: "'Playfair Display',Georgia,serif" }}>{matchScore.w} – {matchScore.b}</div>
            <div style={{ color: "#4a5a6a", fontSize: 10 }}>Cube: {game.cubeValue}×</div>
            {wagerPerPoint > 0 && <div className="sol-pulse" style={{ color: "#9945FF", fontSize: 11 }}>Pot: {(lobby?.totalPot || 0).toFixed(4)} SOL</div>}
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <div style={{ textAlign: "right" }}>
              <div style={{ color: "#aaa", fontWeight: 600, fontSize: 14 }}>{guestName}</div>
              {game.turn === BLACK && <div style={{ color: "#f0c040", fontSize: 10, textTransform: "uppercase" }}>Playing</div>}
            </div>
            <span style={{ fontSize: 20 }}>⚫</span>
          </div>
        </div>
      ...
      <div className="fade-in" style={S.controls}>
        {game.phase === "gameover" ? (
          <div style={{ textAlign: "center" }}>
            <p className="win-banner" style={{ color: "#f0c040", fontSize: 22, fontWeight: 700, fontFamily: "'Playfair Display',Georgia,serif", margin: "0 0 8px" }}>
              {game.winner === WHITE ? hostName : guestName} wins {game.winPoints} point{game.winPoints > 1 ? "s" : ""}!
            </p>
          ...
          <p style={{ color: "#f0c040", textAlign: "center", animation: "softPulse 2s ease-in-out infinite" }}>Doubling in progress...</p>
        ) : isMyTurn ? (
      ...
      <div style={{ textAlign: "center", color: "#3a4a5a", fontSize: 11, marginTop: 6 }}>
        You are {myColor === WHITE ? "⚪ White" : "⚫ Black"}{walletAddr && <span> · <span style={{ color: "#9945FF" }}>{walletAddr.slice(0, 4)}...{walletAddr.slice(-4)}</span></span>}
      </div>
    </div>
  );
}

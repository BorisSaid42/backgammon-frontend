...
function iv(board,bW,bB,pl,src,dest,die){const bar=pl===W?bW:bB;if(src===BAR&&bar===0)return false;if(src!==BAR&&bar>0)return false;if(src!==BAR){if(pl===W&&board[src]<=0)return false;if(pl===B&&board[src]>=0)return false}if(dest===OFF){if(!cbo(board,pl))return false;if(src!==BAR){const ed=pl===W?src-die:src+die;if(ed<0||ed>23){if(pl===W){for(let i=src+1;i<=5;i++)if(board[i]>0)return false}else{for(let i=src-1;i>=18;i--)if(board[i]<0)return false}}}return true}if(pl===W&&board[dest]<-1)return false;if(pl===B&&board[dest]>1)return false;return true}
...
export default function App() {
  ...
  const remainingDice = useMemo(() => {
    if (!game) return [];
    const rd = [...game.dice];
    for (const m of pendingMoves) { const i = rd.indexOf(m.die); if (i >= 0) rd.splice(i, 1); }
    return rd;
  }, [game, pendingMoves]);

  const currentValidMoves = useMemo(() => {
    if (!game || game.phase !== "move" || !isMyTurn || awaitingConfirm) return [];
    let b = [...game.board], bw = game.barW, bb = game.barB;
    for (const m of pendingMoves) { [b, bw, bb] = am2(b, bw, bb, myColor, m.from, m.to).slice(0, 3); }
    return gvm(b, bw, bb, myColor, remainingDice);
  }, [game, isMyTurn, pendingMoves, myColor, remainingDice, awaitingConfirm]);
  ...
  function handlePointClick(pi) {
    ...
    if (selectedPoint !== null && validDestinations.includes(pi)) {
      ...
      const [,,, hit] = am2(currentBoard.board, currentBoard.barW, currentBoard.barB, myColor, selectedPoint, pi);
      const np = [...pendingMoves, { from: selectedPoint, to: pi, die, hit }];

      const diceLeft = [...game.dice];
      for (const m of np) { const i = diceLeft.indexOf(m.die); if (i >= 0) diceLeft.splice(i, 1); }

      if (diceLeft.length === 0) {
        // All dice used — show confirmation modal
      ...
      let b = [...game.board], bw = game.barW, bb = game.barB;
      for (const m of np) { [b, bw, bb] = am2(b, bw, bb, myColor, m.from, m.to).slice(0, 3); }
      const future = gvm(b, bw, bb, myColor, diceLeft);
      if (gmm(future) === 0) {
        // No more moves — show confirmation
        setPendingMoves(np); setSelectedPoint(null); setValidDestinations([]);
        setAwaitingConfirm(true);
        return;
      }
    ...
    const bar = myColor === W ? currentBoard.barW : currentBoard.barB;
    if (pi === BAR || bar > 0) return;
    const piece = currentBoard.board[pi];
    if ((myColor === W && piece <= 0) || (myColor === B && piece >= 0)) return;
    const dests = new Set();
    for (const ms of currentValidMoves) { if (ms.length > 0 && ms[0].from === pi) dests.add(ms[0].to); }
  ...
  function handleBarClick(c) {
    ...
    if (!bar) return;
    const dests = new Set();
    for (const ms of currentValidMoves) { if (ms.length > 0 && ms[0].from === BAR) dests.add(ms[0].to); }
    if (!dests.size) return;
    setSelectedPoint(BAR); setValidDestinations([...dests]);
  }

  function findDie(from, to) {
    for (const ms of currentValidMoves) { if (ms.length > 0 && ms[0].from === from && ms[0].to === to) return ms[0].die; }
  ...
  }, [isMyTurn, game?.phase, currentBoard, pendingMoves.length, awaitingConfirm]);
